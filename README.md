...
### バックエンド
  ...
  - ログイン(新規登録)機能
    - [x] ルーティング設定
    - [x] データベース操作の実装とパスワードの比較
    - [x] bcryptを用い、暗号化の実装
    - [x] フロントエンドからリクエストされたデータを受け取り、レスポンスを返す処理の実装
    - [x] Redisを用い、セッションの実装
  ...
  - 全ガチャ場所(マーカー)の表示機能
    - [x] ルーティング設定
    - [x] データベース操作の実装
    - [x] MySQLから取得したマーカーをフロントエンドに返す処理の実装

  - ガチャ場所についての投稿機能(最新の情報等記載)
    - [ ] ルーティング設定
    - [ ] データベース操作の実装
    - [ ] フロントエンドからリクエストされたデータを受け取り、レスポンスを返す処理の実装

  - ガチャ場所についての投稿取得機能
    - [ ] ルーティング設定
    - [ ] データベース操作の実装
    - [ ] MySQLから取得したデータをフロントエンドに返す処理の実装
...
### フロントエンド
  ...
  - マップページ
    ...
    - [x] マーカー詳細のスタイル調整(一部のみ表示されてしまうため、全体表示させる)
    - [ ] マーカーのリスト表示を文字列に変換
    - [ ] 検索バー追加
...
- アカウントページから自身の投稿の編集
  ...
  - バックエンド
    ...
    - (location.go)アカウント名を構造体に追加し、アカウント名でMySQL内を検索するメソッドを追加
    - (getUserLocationController.go)メソッドの操作
    - (router.go)ルーターでエンドポイントを設定
